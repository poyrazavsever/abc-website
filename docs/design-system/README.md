...
Bu dosya, tum componentlerin ayni görünür dili kullanmasi icin tek kaynaktir.
Amac: farkli ekip üyeleri ayni renk, shadow, radius, spacing ve tarz ile component gelistirsin.
...
- Stil kararlarinda tek kaynak `app/globals.css` içindeki `@theme` tokenlaridir.
...
## 3) Yuzey, Radıus ve Shadow

- Kart/panel yuzeyi: `bg-surface`.
- Ikinci seviye yuzey: `bg-surface-muted`.
- Border dili: `border-border`.
- Radıus seviyeleri tutarli olur: `rounded-md` veya `rounded-lg`.
- Shadow dili tutarli olur: hafif yuzeyde `shadow-xs`, öncelikli yuzeyde `shadow-sm`.
...
- Her etkilesimli componentte su durumlar net olmalıdir: default, hover, active, disabled, focus-visible.
- Focus görünürlugu zorunludur; klavye kullanıcısi odagi net gormelidir.
- Hareket/animasyon kisa ve anlamlı olmalı; dikkat dagitici olmamalidir.

## 6) Kullanim Prensibi

- Yeni bir ekran gelistirirken önce mevcut `components/ui` primitive'lerine bak.
- Ihtiyac karsilanmiyorsa primitive'i genislet, ayni isi yapan yeni stil componenti yazma.
- Button, badge, input gibi atomik parcalar proje genelinde tek implementasyon olmalıdir.

## 7) Kisa Kontrol Listesi

Bir component merge edilmeden önce:

- Ortak token dili kullaniliyor mu?
- Renk/radius/shadow mevcut sistemle tutarli mi?
- Focus ve disabled durumlari var mı?
...
- Tum public UI componentleri `components/ui/index.ts` üzerinden export edilir.
...
- `Field`, label / description / error baglarini ve `aria-describedby` zincirini yönetir.
...
- `Toast` ayrı bir UI componenti olarak kopyalanmaz; mevcut toaster altyapisi kullanilir.

## 11) State ve Accessibility Kontrolu

- Her interaktif component `focus-visible` durumu tasimalidir.
- Disabled state sadece görünür degil, davranissal olarak da bloke edilmelidir.
- Error state, sadece renk degisikligi degil `aria-invalid` ve yardimci metin bağlantısı ile sunulmalidir.
...
- Landing ve auth/onboarding layoutlari önce bu primitive'lere gecirilir.
...
- Foundation primitive setinin tamami olusmus olmalı.
- Her primitive icin default/hover/active/disabled/focus-visible davranisi olmalı.
- Token disi stil (raw hex, keyfi variable utility) olmamali.
- En az iki farkli ekran grubunda primitive kullanimina gecilmis olmalı.
